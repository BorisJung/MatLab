...
% and sets property names to the corresponding property values.
% MMPOLAR(Theta1,Rho1,S1,...,P) plots all the defined curves, and uses the
% structure P having fieldnames equal to property names to set corresponding
% property values contained in the associated fields.
...
%
% MMPOLAR works with HOLD, TITLE, and ZOOM, but not with AXIS or GRID
%
...
for k=1:D.NumLines                 % plot the normalized data
   ...
   D.HLines(k)=plot(HAxes,xdata,ydata,...
                  ...
                  'Marker',D.Marker{k},...
                  'LineWidth',2);   %added by SCF 05.04.2006 
end
if HoldIsON
   set([HFig,HAxes],'NextPlot','add') % hold on
else
   set([HFig,HAxes],'NextPlot','replace')
end
...
if NumRTick>6
   ...
   else % even number, add one lower tick, then keep alternate ones
