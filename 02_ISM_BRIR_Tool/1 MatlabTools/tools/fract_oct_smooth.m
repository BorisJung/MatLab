...
%
% There are two possible 'operational modes' to choose from: 'schaerer' resp. 'welti'
%
% call:
%
% sm = fract_oct_smooth(x,[operation,fs,frac,filterlen,mode,band,b,do_plot]);
%
%     operation     - 'welti'(default)       
%
%     output:
%        sm        - smoothed single-sided spectrum with the
%                    same length as the input spectrum
%     input:
%        x         - input vector/matrix to be smoothed (complex/mag 1-sided spectrum)
...
%         - misses desciption of input x (impulse response) dimension/orientation
%
...
function sm = fract_oct_smooth(x,operation,fs,frac,filterlen,mode,band,b,do_plot)
...
if strcmp(operation,'welti') == 1
    ...
    %-------------------------------------------------------------------------%
...
else if strcmp(operation,'schaerer') == 1
        ...
        h = pre_windowing(x,filterlen);
        H = fft(x,filterlen);
        frac=1/frac;

        if strcmp(mode,'amp') == 1
            H2 = abs(H).^2;
            display('Amplitude smoothing')
        end
        if strcmp(mode,'cmp') == 1
            H_r = real(H);
            H_im = imag(H);
            display('Complex smoothing')
        end
        if strcmp(mode,'eqcmp') == 1
            H_r = real(H);
            H_im = imag(H);
            H2 = abs(H).^2;
            display('Equivalent complex smoothing')
        end
        ...
        switch band
            ...
            case 'oct'
                display(strcat('bandwidth: ',num2str(frac),' octave'))
            ...
            case 'bark'
                display('bandwidth: critical band')
                bandwidth = zeros(filterlen/2+1,1);
                for k = 1:filterlen/2+1
...
if exist('f') %frequency argument vector
    df = f(2);
